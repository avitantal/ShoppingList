-- Fix: deduplicate rows by barcode before staging upsert.
-- When the same barcode appears twice in one batch, Postgres raises
-- "ON CONFLICT DO UPDATE command cannot affect row a second time".
-- DISTINCT ON (barcode) keeps the entry with the latest updated_at.
...
BEGIN
  ...
  -- Stage this batch, deduplicating on barcode (keep latest updated_at per barcode)
  INSERT INTO shopping.staging_prices (barcode, chain_code, item_name, price, updated_at)
  SELECT DISTINCT ON (barcode)
    barcode, p_chain_code, item_name, price, updated_at
  FROM (
    SELECT
      r->>'barcode'                        AS barcode,
      r->>'item_name'                      AS item_name,
      (r->>'price')::numeric(10,2)         AS price,
  ...
  -- Clean staging for this chain only
