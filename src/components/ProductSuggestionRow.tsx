...
export function ProductSuggestionRow({
  ...
  return (
    <li
      ...
      className={cn(
        'grid grid-cols-[minmax(0,1fr)_5rem] gap-2 px-3 py-2.5 cursor-pointer text-sm border-b border-border last:border-b-0',
    ...
    >
      <div className="min-w-0">
        <div className="flex items-start gap-1.5 font-semibold text-text min-w-0">
          {product.previously_bought && (
            <Star size={12} className="shrink-0 mt-1 text-amber-400 fill-amber-400" aria-label="נקנה בעבר" />
          )}
          <span className="line-clamp-2 break-words leading-snug">{product.name}</span>
        </div>
      ...
      <div className="shrink-0 flex flex-col items-start gap-1">
        <span
          className="text-[10px] font-bold px-1.5 py-0.5 rounded max-w-full truncate"
